...
def load_contractfile(fn):
    ...
    re_code_value = (
        r"<pre class='wordwrap' id='js-copytextarea' style='max-height: 250px; margin-top: 5px;'>([\s\S]+?)<\/pre>"
    )
...
def load_txfile(fn):
    ...
    re_str_value = (
        r"<span title=\"The amount of ETH to be transferred to the recipient with the transaction\">\n(.+?)<\/span>"
    )
