...
def gen_contract_hist():
    ...
    print(
        "Max: %d, Min: %d, Mean: %d, Median: %d"
        % (max(oplengths), min(oplengths), np.mean(oplengths), np.median(oplengths))
    )
...
def gen_txworth_hist(tfile):
    txworth = [
        (transaction["txid"], process_worth_str(transaction["transactions"][0]["worth"])) for transaction in tqdm(tfile)
    ]
...
def gen_contract_tx(tfile):
    ...
    print(
        "Min Balance: %d, Max Balance: %d, Sum: %f, Mean Balance: %f, Median Balance: %f"
        % (np.min(balances), np.max(balances), np.sum(balances), np.mean(balances), np.median(balances))
    )
    ...
    try:
        with open(filen) as f:
            ...
            contract_sats.append(
                (contract_name, explored_paths, pathno, concurrency_pairs, timestamp_dependent, exec_time)
            )
    except ValueError:
        return
...
def get_all():
    ...
    print(
        "%d transaction race, %d timestamp, %d in cstack, %d in reentrancy."
        % (len(t_race), len(ts_depend), len(cstack), len(reentrancy))
    )
    ...
    print(
        "CallStack - %d, Transaction Race - %d, Timestamp Dependence - %d, Reentrancy - %d"
        % (len(new_cstack), len(new_t_race), len(new_ts_depend), len(new_reentrancy))
    )
