...
from tests.mocks.mock_subprocess import MockSubprocessContext, OyenteCommandMocks

# Import with mocked dependencies to avoid import errors
with patch.dict(
    "sys.modules",
    {
        "utils": Mock(run_command=Mock(return_value="")),
        "ast_walker": Mock(),
    },
):
    from oyente.ast_helper import AstHelper

# Import the real AstWalker for testing
...
class TestAstHelper:
    ...
    def test_init_solidity_input_type(self, mock_solidity_ast):
        ...
        with patch('utils.run_command') as mock_run:
            mock_run.return_value = json.dumps(mock_solidity_ast)
            with patch('ast_walker.AstWalker', return_value=AstWalker()):
    ...
    def test_get_source_list_standard_json(self, mock_standard_json, mock_file_content):
        ...
        with patch('builtins.open', mock_file_content(json.dumps(mock_standard_json))):
            with MockSubprocessContext({"solc --combined-json ast  test.json": {"returncode": 0, "stdout": json.dumps({"sources": {}}), "stderr": ""}}):
                helper = AstHelper("test.json", "standard json", "")
    ...
    def test_get_source_list_solidity_no_allow_paths(self, mock_solidity_ast):
        ...
        with MockSubprocessContext({
            "solc --combined-json ast  test.sol": {
                "returncode": 0,
                "stdout": json.dumps(mock_solidity_ast),
                "stderr": ""
            }
        }) as mock_subprocess:
            helper = AstHelper("test.sol", "solidity", "")
            ...
            assert len(mock_subprocess.call_history) > 0
            assert "test.sol" in helper.source_list
    ...
    def test_get_source_list_solidity_with_allow_paths(self, mock_solidity_ast):
        ...
        with MockSubprocessContext({
            "solc --combined-json ast  test.sol --allow-paths /opt/libs": {
                "returncode": 0,
                "stdout": json.dumps(mock_solidity_ast),
                "stderr": ""
            }
        }) as mock_subprocess:
            helper = AstHelper("test.sol", "solidity", "", "/opt/libs")
            
            # Check that the command was called with allow_paths
            assert any("/opt/libs" in str(call["args"]) for call in mock_subprocess.call_history)
    ...
    def test_get_linearized_base_contracts(self, mock_solidity_ast):
        ...
        with patch('oyente.ast_helper.run_command') as mock_run:
            mock_run.return_value = json.dumps(mock_solidity_ast)
            helper = AstHelper("test.sol", "solidity", "")
    ...
    def test_extract_func_call_definitions(self, mock_solidity_ast):
        ...
        mock_ast = mock_solidity_ast.copy()
        func_call = {
            "name": "FunctionCall",
            "id": "4",
        ...
        with patch('oyente.ast_helper.run_command') as mock_run:
            mock_run.return_value = json.dumps(mock_ast)
            helper = AstHelper("test.sol", "solidity", "")
                                ...
                                "children": [
                                    {
                                        ...
                                        "children": [
                                            {
                                                "attributes": {"type": "contract OtherContract"}
                                            }
                                        ]
                                    }
                                ]
    ...
    def test_semi_convert_new_to_old_ast_format(self, mock_v5_ast):
        ...
        with patch('oyente.ast_helper.run_command') as mock_run:
            mock_run.return_value = json.dumps(mock_v5_ast)
            helper = AstHelper("test.sol", "solidity", "")
...
class TestAstHelperEdgeCases:
    ...
    @patch('oyente.ast_helper.run_command')
    def test_malformed_json_from_solc(self, mock_run_command):
    ...
    def test_missing_standard_json_file(self, mock_open):
        """Test handling of missing standard JSON file."""
        with pytest.raises(FileNotFoundError):
            AstHelper("test.json", "standard json", "")

    @patch('oyente.ast_helper.run_command')
    def test_empty_ast_sources(self, mock_run_command):
        """Test handling of empty AST sources."""
        mock_run_command.return_value = json.dumps({"sources": {}})
    ...
    @patch('oyente.ast_helper.run_command')
    def test_ast_without_contracts(self, mock_run_command):
        ...
        mock_run_command.return_value = json.dumps(mock_ast)
    ...
    @patch('oyente.ast_helper.run_command')
    def test_contract_without_children(self, mock_run_command):
    ...
    @patch('oyente.ast_helper.run_command', side_effect=Exception("Solc error"))
    def test_solc_command_failure(self, mock_run_command):
...
class TestAstHelperIntegration:
                                ...
                                "children": [
                                    {
                                        "name": "VariableDeclaration",
                                        "id": "2",
                                        "attributes": {"name": "totalSupply"}
                                    },
                                    {
                                        "name": "VariableDeclaration",
                                        "id": "3",
                                        "attributes": {"name": "balances"}
                                    },
                                    {
                                        "name": "FunctionDefinition",
                                        "id": "4",
    ...
    @patch('oyente.ast_helper.run_command')
    def test_complex_contract_analysis(self, mock_run_command, complex_ast):
        """Test analysis of a more complex contract structure."""
        mock_run_command.return_value = json.dumps(complex_ast)
