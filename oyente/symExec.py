...
def collect_vertices(lines: List[str]) -> None:
    ...
    for line in lines:
        ...
        logging.debug(
            "Before processing: current_block=%05x, current_ins_address=%05x, is_new_block=%s",
            current_block,
            current_ins_address,
            is_new_block,
        )
    ...
    if current_block not in end_ins_dict:
        logging.debug(
            "Final block %05x did not have an end; setting end_ins_dict[%05x] = %05x",
            current_block,
            current_block,
            current_ins_address,
        )
...
def sym_exec_ins(params, block, instr, func_call, current_func_name):
    ...
    #
        ...
        if len(stack) > 0:
            ...
            if isAllReal(address, current_miu_i) and address in mem:
                temp = long(math.ceil((address + 32) / float(32))) if six.PY2 else math.ceil((address + 32) / float(32))
                if temp > current_miu_i:
                    current_miu_i = temp
                value = mem[address]
    ...
    # see https://github.com/enzymefinance/oyente/commit/26cf59c926889b3e56e6e7ed0fd50dcc71aebfb3
        ...
        if len(stack) > 1:
            ...
            stored_value = stack.pop(0)
            logging.info(
                "Analyzed opcode TSTORE with values: stored_address=%s, stored_value=%s", stored_address, stored_value
            )
            if isReal(stored_address):
